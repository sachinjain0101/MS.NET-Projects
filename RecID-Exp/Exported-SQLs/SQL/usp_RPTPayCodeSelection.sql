...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.usp_RPTPayCodeSelection') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE dbo.usp_RPTPayCodeSelection AS' 
END 
 GO

ALTER    PROCEDURE [dbo].[usp_RPTPayCodeSelection]
(
  @DateFrom DATETIME,
  @DateTo DATETIME,
  @Date DATETIME,
