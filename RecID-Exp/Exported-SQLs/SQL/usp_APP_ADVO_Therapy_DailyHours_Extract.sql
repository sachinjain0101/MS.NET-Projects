...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.usp_APP_ADVO_Therapy_Actual_DailyHours_Extract') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE dbo.usp_APP_ADVO_Therapy_Actual_DailyHours_Extract AS' 
END 
 GO

ALTER PROCEDURE  usp_APP_ADVO_Therapy_Actual_DailyHours_Extract
 @client VARCHAR(4)
, @GroupCode INT 
