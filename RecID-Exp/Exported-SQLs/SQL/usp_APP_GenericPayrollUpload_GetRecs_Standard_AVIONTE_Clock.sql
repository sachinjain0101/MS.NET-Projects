USE [TimeHistory]
GO
/****** Object:  StoredProcedure [dbo].[usp_APP_GenericPayrollUpload_GetRecs_Standard_AVIONTE_Clock]    Script Date: 5/14/2015 3:09:09 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_APP_GenericPayrollUpload_GetRecs_Standard_AVIONTE_Clock]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[usp_APP_GenericPayrollUpload_GetRecs_Standard_AVIONTE_Clock] AS' 
END
GO

Alter PROCEDURE [dbo].[usp_APP_GenericPayrollUpload_GetRecs_Standard_AVIONTE_Clock]
(
  @Client       varchar(4),
  @GroupCode    int,
  @PPED         datetime,
...
Declare @AgencyList varchar(30)
Declare @iLoc int
Set @AgencyList = 'ALL'

IF @PayrollType like '%AGENCY=%'
BEGIN
  Set @iLoc = charindex('AGENCY=',@PayrollType)  
  Set @AgencyList = substring(@PayrollType,@iLoc+7,12) + ',99'
END
...
SELECT 	
    ...
    , [AssignmentNo] = CASE WHEN LTRIM(ISNULL([eh].[AssignmentNo],'')) = '' 
    ...
    FROM TimeHistory.dbo.tblTimeHistDetail as hd
        ...
        Left JOIN TimeHistory.[dbo].[tblEmplNames_depts] as eh
            ON	[eh].Client = [hd].Client
            AND	[eh].GroupCode = [hd].GroupCode
						AND [eh].PayrollPeriodEndDate = hd.PayrollPeriodEndDate
            AND	[eh].SSN = [hd].SSN
            AND [eh].Department = [hd].DeptNo
    ...
    , CASE WHEN LTRIM(ISNULL([eh].[AssignmentNo],'')) = '' 
                            THEN CASE WHEN LTRIM(ISNULL([ed].[AssignmentNo],'')) = '' 
                                      THEN CASE WHEN ISNULL(hd.CostID,'') = '' 
                                                THEN 'MISSING' 
                                                ELSE hd.costID 
...
BEGIN
  ...
  SELECT 	
    ...
    FROM TimeHistory.dbo.tblTimeHistDetail_UDF as udf
        ...
        Left JOIN TimeHistory.[dbo].[tblEmplNames_depts] as eh
            ON	[eh].Client = [udf].Client
            AND	[eh].GroupCode = [udf].GroupCode
						AND [eh].PayrollPeriodEndDate = @PPED
            AND	[eh].SSN = [udf].SSN
            AND [eh].Department = [udf].DeptNo
        /*Left JOIN TimeCurrent.[dbo].[tblEmplAssignments] as edh
