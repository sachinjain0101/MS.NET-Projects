CREATE PROCEDURE [dbo].[usp_APP_GenericPayrollUpload_GetRecs_BRNS]
(
  @Client       varchar(4),
  @GroupCode    int,
  @PPED         datetime,
...
SELECT 	
    ...
    , [DeptCode] = ISNULL(gd.ClientDeptCode,'')
    , [DeptCode2]= ISNULL(gd.ClientDeptCode2,'')
    ...
    FROM TimeHistory.dbo.tblTimeHistDetail as hd
        ...
        ON hd.Client = gd.Client
        AND hd.GroupCode = gd.GroupCode
        AND hd.DeptNo = gd.DeptNo
    WHERE	[hd].Client = @Client 
        ...
        AND [hd].SSN NOT IN ( SELECT DISTINCT SSN
