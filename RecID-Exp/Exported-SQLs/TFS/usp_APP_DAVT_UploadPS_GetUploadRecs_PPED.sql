Create PROCEDURE [dbo].[usp_APP_DAVT_UploadPS_GetUploadRecs_PPED]
(
  @Client     char(4), 
  @GroupCode  int, 
  @PPED       datetime,
...
--print 'After Checks Elapse Time (secs) = ' + ltrim(str(datediff(second,@StartTime,getdate())))
...
SELECT RecordID, PayrollPeriodEnddate, MasterPayrollDate,
  ...
  ShiftNo, AgencyNo, ISNULL(ClockAdjustmentNo,''),
...
WHERE thd.Client = @Client 
  ...
  AND IsNull(thd.CrossoverStatus, '') not in('2','1')         -- Skip cross over records and immediate pay records
  AND ISNULL(thd.ClockAdjustmentNo,'') IN('1','8','Q','R','S','M','T','U','V','Z','O','',' ')
  AND thd.Holiday = '0'


--print 'After Load Work table Elapse Time (secs) = ' + ltrim(str(datediff(second,@StartTime,getdate())))
...
--print 'After Travel Fixup Elapse Time (secs) = ' + ltrim(str(datediff(second,@StartTime,getdate())))
 ...
 DECLARE cTravel CURSOR
  ...
  and e.groupcode = t.groupcode
  and e.ssn = t.ssn
  Left Join TimeCurrent..tblDeptNames as d
...
--print 'After Travel Fixup 2 Elapse Time (secs) = ' + ltrim(str(datediff(second,@StartTime,getdate())))
  ...
  AND thd2.RecordID = (
    ...
    WHERE thd3.DeptNo <> 88
      ...
      AND (dbo.PunchDateTime2(thd.TransDate, thd.OutDay, thd.OutTime) = dbo.PunchDateTime2(thd3.TransDate, thd3.InDay, thd3.InTime)
           OR
           dbo.PunchDateTime2(thd.TransDate, thd.OutDay, dateadd(minute,1,thd.OutTime)) = dbo.PunchDateTime2(thd3.TransDate, thd3.InDay, thd3.InTime)
           )
      AND thd3.Hours > 0
  )
*/  
...
Inner JOIN TimeCurrent..tblEmplNames as en  with (nolock)
  ...
  AND en.SSN = thd.SSN
Inner JOIN TimeCurrent..tblGroupDepts AS dxr 
...
WHERE thd.Client = @Client 
  AND thd.GroupCode = @GroupCode
  AND thd.PayrollperiodEndDate = @PPED AND (@Accrual = 0 OR thd.TransDate <= @CutoffDate)
  AND thd.AprvlStatus not in('2','1') -- Skip any records that were copied to the employees primary group or are immediate pay records
																		  -- AprvlStatus is correct in this case since its selecting from the work table
...
--print 'After Hours Load Elapse Time (secs) = ' + ltrim(str(datediff(second,@StartTime,getdate())))
...
WHERE thd.Client = @Client 
  AND thd.GroupCode = @GroupCode
  AND thd.PayrollperiodEndDate = @PPED AND (@Accrual = 0 OR thd.TransDate <= @CutoffDate)
  AND IsNull(thd.CrossoverStatus, '') not in('2','1')        -- Skip any records that were copied to the employees primary group or are immediate pay records
...
--print 'After Non-Worked Adjs load Elapse Time (secs) = ' + ltrim(str(datediff(second,@StartTime,getdate())))
...
--print 'After Worked Adjs load Elapse Time (secs) = ' + ltrim(str(datediff(second,@StartTime,getdate())))
...
BEGIN
  ...
  and t.groupcode = @Groupcode 
  and t.PayrollPeriodenddate = @PPED
  AND isnull(t.CrossOverStatus,'') not in('2','1')
  --and t.ssn = 289768097
  group By t.SSN
...
--print 'After Adjs/Hours Combine load Elapse Time (secs) = ' + ltrim(str(datediff(second,@StartTime,getdate())))
...
cast(Replace(en.FirstName, ',', '') as varchar(100)) as FirstName,
cast(Replace(en.LastName, ',', '')as varchar(100))  as LastName,
en.PayType,
...
--print 'After Final load Elapse Time (secs) = ' + ltrim(str(datediff(second,@StartTime,getdate())))
...
Update #tmpFinal
  Set #tmpFinal.JobCode = isnull(x.XRefValue,'XXXX')
from #tmpFinal
Left Join TimeCurrent.[dbo].[tblClientXRef] as x
on x.Client = @Client 
and x.XRefType = 80
and x.xrefID = right(#tmpFinal.UploadCode,3) -- Get the financial class from the upload value
...
BEGIN
	...
	select
		...
		,''
		,''
		,Upper(en.AssignmentNo)
		,CASE CAST(en.AgencyNo AS varchar(1)) WHEN '1' THEN 'X' WHEN '2' THEN 'Z' WHEN '3' THEN 'Y' ELSE 'N' END AS HomeShiftNo
  ...
  AND t.ClockAdjustmentNo IN('1','8','Q','R','S','M','T','U','V','Z','O','',' ')	
	AND isnull(sn.CLientFacility,'') like 'Perdiem:%'
	AND en.AgencyNo < 5
	Group by
		 t.Payrollperiodenddate
		,en.PrimarySite
...
--print 'After last select Elapse Time (secs) = ' + ltrim(str(datediff(second,@StartTime,getdate())))
