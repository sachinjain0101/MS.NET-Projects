USE [TimeHistory]
GO
/****** Object:  StoredProcedure [dbo].[usp_Web1_GetMissingPunchesEE]    Script Date: 5/12/2015 3:07:56 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_Web1_GetMissingPunchesEE]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[usp_Web1_GetMissingPunchesEE] AS' 
END
GO

 --  EXEC usp_Web1_GetMissingPunchesEE 14820, 'PRID', 369906, '06/21/15', 2, 4, 491864937 

ALTER  proc [dbo].[usp_Web1_GetMissingPunchesEE] (
  @UserID           int,
  @Client           varchar(4),
  @GroupCode        int,
  @PPED             datetime,
...
INSERT INTO @tmpMPs
  SELECT thd.RecordID,
    ...
    null,null,
    thd.TransDate,thd.TransType, thd.InDay, thd.InTime, thd.OutDay, thd.OutTime, 
