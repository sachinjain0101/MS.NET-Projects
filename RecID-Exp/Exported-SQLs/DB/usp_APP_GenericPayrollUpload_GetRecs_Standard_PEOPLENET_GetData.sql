CREATE PROCEDURE [dbo].[usp_APP_GenericPayrollUpload_GetRecs_Standard_PEOPLENET_GetData]
(
    @Client         VARCHAR(4),
    @GroupCode      INT,
    @PPED           DATETIME,
...
(
    ...
    , MinTHDRecord           BIGINT  --< MinTHDRecordId data type is changed from  INT to BIGINT by Srinsoft on 11Aug2016 >--
...
INSERT INTO #tmpExport
   SELECT     
		...
		, [ApproverName] = ''
		, [ApproverEmail] = ''
		, [ApprovalDate] = MAX([hd].[AprvlStatus_Date])
    ...
    FROM 
        ...
        LEFT JOIN [TimeCurrent].[dbo].[tblUser] u
            ON [u].[UserID]=[hd].[AprvlStatus_UserID]
 ...
 SELECT     
   ...
   WHERE   [hd].[Client] = @Client 
    ...
    GROUP BY 
        ...
        , ISNULL([edh].[BilltoCode],' ')
        , [enh].[PayRecordsSent]
--        , ISNULL([hd].[AprvlStatus_UserID],0)
...
UPDATE #tmpExport
SET #tmpExport.ApproverName = ISNULL(usr.FirstName, ISNULL(edh.Approver_FirstName1,ISNULL(edh.Approver_FirstName2,' ')) )
                                 +' '+ ISNULL(usr.LastName, ISNULL(edh.Approver_LastName1, ISNULL(edh.Approver_LastName2, ' ') ) ),
    #tmpExport.ApproverEmail = ISNULL(usr.Email, edh.Approver_Email1),
    #tmpExport.Web_Approver_ID = ISNULL(thd.AprvlStatus_UserID,0)
FROM #tmpExport
...
AND edh.SiteNo=thd.SiteNo
AND edh.SSN = thd.SSN
AND edh.DeptNo = thd.DeptNo
