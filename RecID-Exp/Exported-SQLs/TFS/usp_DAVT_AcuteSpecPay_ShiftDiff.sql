Create PROCEDURE [dbo].[usp_DAVT_AcuteSpecPay_ShiftDiff]
(
  @PRSiteNo int,
  @Client varchar(4),
  @GroupCode int,
...
ClockInTime = isnull(ActualInTime, TimeHistory.dbo.PunchDateTime2(TransDate, inDay, Intime)),
ClockOutTime = isnull(ActualOutTime, TimeHistory.dbo.PunchDateTime2(TransDate, outDay, OutTime)),
isnull(InClass,'S'), isnull(OutClass,'S'), DeptNo, SiteNo, masterpayrolldate, isnull(Changed_DeptNo,'')
from Timehistory..tblTimeHistDetail where client = @Client
and groupcode = @GroupCode
and SSn = @SSN
and Payrollperiodenddate = @PPED
and clockadjustmentno = ''
and DeptNo in(select DeptNo from TimeCurrent..tblDeptShiftDiffs where Client = @Client and GroupCode = @GroupCode and SiteNo = tblTimeHistDetail.SiteNo )
...
BEGIN
    ...
    Select @NewShiftNo = ShiftNo, 
      ...
      BEGIN
        Set @PrevShiftno = isnull(@NewShiftNo,1)
        Set @PrevShiftDiffAmt = isnull(@ShiftDiffAmt,0)
      END
      ELSE
      BEGIN
        Set @PrevShiftno = 0
...
-- Delete old ShiftDiff Adjustments.
-- 
Delete from TimeHistory..tblTimeHistDetail
  where client = @Client and groupcode = @groupCode and SSN = @SSN
    and Payrollperiodenddate = @PPED 
    and ClockAdjustmentNo = 'I' 
--    and ClkTransNo = 406 
    and InSrc = '3' and OutSrc = '3'

-- Calculate the Shift Diff amount.  This needs to be done by Worked Site and Worked Dept.
-- So we need to build another cursor.
--

DECLARE @Dollars numeric(9,2)
DECLARE @Division BIGINT  --< @Division data type is changed from  INT to BIGINT by Srinsoft on 26Oct2016 >--

DECLARE cTHDSum CURSOR
READ_ONLY
FOR 
select SiteNo, DeptNo, Sum( Hours * ShiftDiffAmt ) 
from TimeHistory..tblTimeHistdetail
where client = @Client 
  and groupcode = @groupCode 
  and SSN = @SSN
  and Payrollperiodenddate = @PPED 
  and ShiftNo > 0  --and isnull(ShiftDiffAmt,0) > 0 
Group By SiteNo, deptNo

OPEN cTHDSum

FETCH NEXT FROM cTHDSum INTO @SiteNo, @deptNo, @Dollars
WHILE (@@fetch_status <> -1)
BEGIN
	IF (@@fetch_status <> -2)
	BEGIN
...
if @Client = 'DVPC' 
BEGIN
  EXEC TimeHistory.dbo.usp_DAVT_SpecPay_Preceptor_Bonus @Client, @GroupCode, @PPED, @SSN
  RETURN
END
--EXEC [TimeHistory].[dbo].[usp_APP_DAVT_CA_BreakPenalty_SpecPay] @Client, @GroupCode, @PPED, @SSN
