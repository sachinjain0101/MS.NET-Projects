CREATE PROCEDURE [dbo].[usp_APP_PuertoRicoBreakRules_Clock]
(
	  @DefaultMealDuration NUMERIC(7,2) 
	, @DefaultBreakRule    INT
  	, @Client 				VARCHAR(4)
	, @GroupCode  	  INT
 	, @PPED   				DATETIME
	, @SSN					  INT	
...
DECLARE @BreakClockAdjustmentNo VARCHAR(1)
...
PRINT '@DisableBreakExceptions: ' + CAST(@DisableBreakExceptions AS VARCHAR)

IF (ISNULL(@BreakRuleID, 0) = 0)
BEGIN
	SET @BreakRuleID = @DefaultBreakRule
END
...
IF ((ISNULL(@BreakRuleID, 0) = 0 OR @DisableBreakExceptions = '1') AND @Client <> 'OLST')
BEGIN
	PRINT 'Break Rule Not Found'
	RETURN
END

DECLARE @MealDuration NUMERIC(9,2);
SELECT TOP 1 @mealduration = ea.MealDuration 
...
IF (ISNULL(@MealDuration, 0) = 0)
BEGIN
	SET @MealDuration = @DefaultMealDuration
END
PRINT '@MealDuration: ' + CAST(@MealDuration AS VARCHAR)

INSERT INTO #tmpPunch(MinTHDRecordID, SiteNo, DeptNo, MealPermit, [Hours], TransDate, InTime, ActualInTime, OutTime)
SELECT MIN(thd.RecordID), thd.SiteNo, thd.DeptNo, @MealDuration, SUM(thd.[Hours]), thd.TransDate, dbo.PunchDateTime2(thd.TransDate, thd.InDay, thd.InTime), thd.ActualInTime, dbo.PunchDateTime2(thd.TransDate, thd.OutDay, thd.OutTime)
