CREATE Procedure [dbo].[usp_APP_GenericDataExtract_Peoplenet_DLT_MS2] (
...
UPDATE #tmpSummary
SET ApprovalStatus = CASE	WHEN DisputedCount > 0 THEN 'PENDING'
							WHEN ApprovedCount = TxnCount THEN 'APPROVED'
							ELSE 'UNAPPROVED' END

UPDATE #tmpSummary
SET AssignmentNo = 'None'
WHERE ISNULL(AssignmentNo, '') = ''
...
SELECT	  'EmplID' + @Delimeter 
	  ...
	  , ApprovalStatus = ''
UNION
SELECT	TimeHistory.dbo.fn_WrapVarChar(FileNo, @DoubleQuote) + @Delimeter +
