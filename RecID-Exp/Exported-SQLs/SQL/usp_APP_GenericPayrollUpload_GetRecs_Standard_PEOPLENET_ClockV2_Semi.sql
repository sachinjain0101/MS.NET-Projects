USE [TimeHistory]
GO
/****** Object:  StoredProcedure [dbo].[usp_APP_GenericPayrollUpload_GetRecs_Standard_PEOPLENET_ClockV2_Semi]    Script Date: 8/27/2015 12:45:52 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
--select * from TimeHistory..tblTimeHistDetail where Client = 'PENM' and GroupCode = 290001
----exec usp_APP_GenericPayrollUpload_GetRecs_Standard_PEOPLENET_ClockV2 'HSSS', 230238, '10/25/13', 'NO','FileNo','REG','OT','DT','Avionte','N','Y'
----EXEC usp_APP_GenericPayrollUpload_GetRecs_Standard_PEOPLENET_GetDataV2 'HSSS', 230238, '10/25/13', '11/26/13', 'NO','FileNo','REG','OT','DT','Avionte','N','Y' --'CORN',112,'03/22/2015','03/29/2015','No','SSN','REG','OT','DT','Avionte','N','Y'

----exec usp_APP_GenericPayrollUpload_GetRecs_Standard_PEOPLENET_ClockV2_Semi 'PENM', 777501, '02/21/16', 'NO','FileNo','REG','OT','DT','PeoplenetV2','N','Y'


--select * from TimeHistory..tblTimeHistDetail where Client = 'CORN' and GroupCode = 112
----EXEC usp_APP_GenericPayrollUpload_GetRecs_Standard_PEOPLENET_GetDataV2 'CORN',112,'03/22/15','03/22/15','No','FileNo','REG','OT','DT','Avionte','N','Y'
----exec usp_APP_GenericPayrollUpload_GetRecs_Standard_PEOPLENET_ClockV2 'CORN', 112, '03/22/15', 'NO','FileNo','REG','OT','DT','Avionte','N','Y'
ALTER PROCEDURE [dbo].[usp_APP_GenericPayrollUpload_GetRecs_Standard_PEOPLENET_ClockV2_Semi]
(
  @Client       varchar(4),
  @GroupCode    int,
  @PPED         datetime,
...
BEGIN
    ...
    DECLARE @StartTrans datetime
    DECLARE @EndTrans datetime
    DECLARE @StartPPED datetime
    DECLARE @EndPPED datetime
    DECLARE @TempMonth INT
    DECLARE @SemiMo1 INT
    DECLARE @SemiMo2 INT
  
    Set @StartPPED = dateadd(day, -18, @PPED)
    Set @EndPPED = dateadd(day, 8, @PPED)

		SELECT @SemiMo1 = SemiMoPayrollDay1, @SemiMo2 = SemiMoPayrollDay2
		FROM timecurrent..tblClientGroups
		WHERE client = @Client
		AND GroupCode = @GroupCode

		IF DAY(@pped) = @SemiMo1  --assume @SemiMo1 is always less than @SemiMo2
			BEGIN
				IF @SemiMo2 = 31  --end of month
				   Set @StartTrans = ltrim(str(month(@PPED))) + '/01/' + ltrim(str(year(@PPED)))
				ELSE
		     		 Set @StartTrans = DATEADD(dd, 1, DATEADD(mm, -1, ltrim(str(month(@PPED))) + '/'+ LTRIM(STR(@semiMo2)) +'/' + ltrim(str(year(@PPED)))))
				Set @EndTrans = @pped
			END				
		ELSE
			BEGIN
				Set @StartTrans = DATEADD(dd, 1, ltrim(str(month(@PPED))) + '/'+ LTRIM(STR(@semiMo1)) +'/' + ltrim(str(year(@PPED))))
				Set @EndTrans = @pped
			END
    /*
    Print @StartPPED
    ...
    */
    ...
    if @TestingFlag = 'N'
    BEGIN
      EXEC [TimeHistory].[dbo].[usp_APP_PRECHECK_Upload_Semimonthly] @Client, @GroupCode, @PPED, 'N'
    END
END
--semimonthly payfile 
...
SELECT 	
    ...
    WHERE	[hd].Client = @Client 
		...
		AND	hd.PayrollPeriodEndDate >= @StartPPED
        AND	hd.PayrollPeriodEndDate <= @EndPPED
        AND hd.TransDate >= @StartTrans
        AND hd.TransDate <= @EndTrans
