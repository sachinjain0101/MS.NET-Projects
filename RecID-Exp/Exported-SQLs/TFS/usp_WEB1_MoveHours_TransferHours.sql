Create PROCEDURE [dbo].[usp_WEB1_MoveHours_TransferHours]
(
  @Client varchar(4),
  @GroupCode int,
  @PPED datetime,
...
BEGIN TRANSACTION
  ...
  INSERT INTO [TimeHistory].[dbo].[tblTimeHistDetail]
        ([Client], [GroupCode], [SSN], [PayrollPeriodEndDate], [MasterPayrollDate], [SiteNo], [DeptNo], [JobID], [TransDate], [EmpStatus], [BillRate], [BillOTRate], [BillOTRateOverride], [PayRate], [ShiftNo], [InDay], [InTime],           [OutDay], [OutTime],          [Hours],       [Dollars], [ClockAdjustmentNo], [AdjustmentCode], [AdjustmentName], [TransType], [AgencyNo], [InSrc], [OutSrc], [DaylightSavTime], [Holiday], [AprvlStatus], [AprvlStatus_UserID], [AprvlStatus_Date], [AprvlAdjOrigRecID], [HandledByImporter], [AprvlAdjOrigClkAdjNo], [ClkTransNo], [ShiftDiffClass], [Borrowed], [UserCode],[OutUserCode], [DivisionID], [CostID], [ShiftDiffAmt],[ActualInTime],[ActualOutTime] )
  Select TOP 1 [Client], [GroupCode], [SSN], [PayrollPeriodEndDate], [MasterPayrollDate], [SiteNo], [DeptNo], [JobID], [TransDate], [EmpStatus], [BillRate], [BillOTRate], [BillOTRateOverride], [PayRate], [ShiftNo], [InDay], [InTime], [OutDay], [OutTime], (-1 * @Hours), [Dollars], '1',                 [AdjustmentCode], case when @Client in('DVPC','DAVT') then 'MOVED' else 'WORKED' end,         101,         [AgencyNo], '3',     '3',      [DaylightSavTime], [Holiday], '', 0, NULL, [AprvlAdjOrigRecID], 'V',                 [AprvlAdjOrigClkAdjNo], [ClkTransNo], [ShiftDiffClass], [Borrowed], @UserCode,@UserCode,  [DivisionID], [CostID], [ShiftDiffAmt],NULL,NULL
  ...
  INSERT INTO [TimeCurrent].[dbo].[tblAdjustments]([ReverseFlag], [OrigRecord_No], [Client], 
        ...
        [FriVal], [SatVal], [SunVal], [WeekVal], [TotalVal], [UserName], [UserID], 
        [TransDateTime], [DeletedDateTime], [DeletedByUserName], [DeletedByUserID], [SweptDateTime], 
        [RecordStatus], [IPAddr], [ShiftNo],THDRecordID)
  (Select '', RecordID, Client, GroupCode, PayrollPeriodEndDate, SSN, SiteNo, DeptNo, ClockAdjustmentNo,
        ...
        Hours, @UserName,@UserID,getdate(),null,null,null,null,'1','99.99.99.99',ShiftNo,RecordID
  ...
  INSERT INTO [TimeHistory].[dbo].[tblTimeHistDetail]
        ([Client], [GroupCode], [SSN], [PayrollPeriodEndDate], [MasterPayrollDate], [SiteNo], [DeptNo], [JobID], [TransDate], [EmpStatus], [BillRate], [BillOTRate], [BillOTRateOverride], [PayRate], [ShiftNo], [InDay], [InTime],           [OutDay], [OutTime],          [Hours], [Dollars], [ClockAdjustmentNo], [AdjustmentCode], [AdjustmentName], [TransType], [AgencyNo], [InSrc], [OutSrc], [DaylightSavTime], [Holiday], [AprvlStatus], [AprvlStatus_UserID], [AprvlStatus_Date], [AprvlAdjOrigRecID], [HandledByImporter], [AprvlAdjOrigClkAdjNo], [ClkTransNo], [ShiftDiffClass], [Borrowed], [UserCode],[OutUserCode], [DivisionID], [CostID], [ShiftDiffAmt],[ActualInTime],[ActualOutTime])
  Select TOP 1 [Client], [GroupCode], [SSN], [PayrollPeriodEndDate], [MasterPayrollDate],CASE WHEN @NewSite = 0 THEN [SiteNo] ELSE @NewSite END, @NewDept, [JobID], [TransDate], [EmpStatus], [BillRate], [BillOTRate], [BillOTRateOverride], [PayRate], [ShiftNo], [InDay], [InTime], [OutDay], [OutTime],  @Hours, [Dollars], '1',                 [AdjustmentCode], case when @Client in('DVPC','DAVT') then 'MOVED' else 'WORKED' end,         101,         [AgencyNo], '3',     '3',      [DaylightSavTime], [Holiday], '', 0, NULL, [AprvlAdjOrigRecID], 'V',                 [AprvlAdjOrigClkAdjNo], [ClkTransNo], [ShiftDiffClass], [Borrowed], @UserCode, @UserCode, [DivisionID], [CostID], [ShiftDiffAmt],NULL,NULL
  From TimeHistory..tblTimeHistDetail
  where client = @Client and groupcode = @Groupcode and PayrollPeriodEndDate = @PPED
  and SSN = @SSN and TransDate = @TransDate and DeptNo = @DeptNo and SiteNo = @SiteNo 
  and ShiftNo = @ShiftNo and isnull(ShiftDiffClass,'0') = @ShiftDiffClass
