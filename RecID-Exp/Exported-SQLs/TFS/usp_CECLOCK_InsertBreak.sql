Create PROCEDURE [dbo].[usp_CECLOCK_InsertBreak]
    (
      @TermID VARCHAR(20) = '' ,
      @EmplBadge INT = 0 ,
      @DeptNo INT ,
...
-- 
IF @Client = 'COAS' and @NewAmount <= 0.33
BEGIN
  Set @BreakAmount = 0
  ...
  BEGIN
    --Set @Comment = 'Employee entered a break amount of ' + LTRIM(str(@newAmount,4,2)) + ' which is less than 30 minutes(0.50) and the break amount has been reset to zero.'
		Set @Comment = 'Employee entered a break amount of ' + LTRIM(str(@newAmount,4,2)) + ' which is less than 20 minutes(0.33) and the break amount has been reset to zero.'
