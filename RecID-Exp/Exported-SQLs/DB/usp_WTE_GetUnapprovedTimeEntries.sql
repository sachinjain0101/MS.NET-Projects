CREATE PROCEDURE [dbo].[usp_WTE_GetUnapprovedTimeEntries] 
...
BEGIN
  ...
  SELECT DISTINCT thd.RecordID
	      ...
	      , sn.SiteName
	      , ISNULL(gd.DeptName_Long, '') as DeptName
	      ...
	      , CASE ISNULL(tmpAss.TimeEntryFreqID, 2) WHEN 5 THEN CONVERT(VARCHAR(25), DATEADD(dd, -(DAY(thd.TransDate)-1), thd.TransDate), 101)
...
BEGIN
  ...
  SELECT DISTINCT thd.RecordID
	      ...
	      , CASE ISNULL(tmpAss.TimeEntryFreqID, 2)
		      WHEN 5 THEN CONVERT(VARCHAR(25), DATEADD(dd, -(DAY(thd.TransDate)-1), thd.TransDate), 101)
		      ELSE thd.PayrollPeriodEndDate 
	        END AS ParentPayrollDate
	      , ISNULL(cgs.DisputeMode, 0) as DisputeMode
		    , '' as Comments
...
BEGIN
  ...
  */
  ...
  -- Monthly	
	...
	BEGIN
	  ...
	  INNER JOIN TimeHistory.dbo.tblWTE_Spreadsheet_Assignments sa WITH(NOLOCK)
		...
		AND t.TransDate BETWEEN CONVERT(VARCHAR(25), DATEADD(dd, -(DAY(ts.TimesheetEndDate)-1), ts.TimesheetEndDate), 101) AND ts.TimesheetEndDate
  ...
  LEFT JOIN TimeCurrent..tblAdjCodes adj
	...
	CROSS APPLY TimeHistory.dbo.fn_GetApprovalBillableFlags(t.Client, t.GroupCode, t.SiteNo, @MethodID, adj.Worked, adj.Billable, adj.Payable, adj.AdjustmentType) flg
...
BEGIN
  ...
  SELECT t.THDRecordID AS RecordID
      ...
      , CASE ISNULL(t.TimeEntryFreqID, 2)
	        WHEN 5 THEN CONVERT(VARCHAR(25), DATEADD(dd, -(DAY(t.TransDate) -1), t.TransDate), 101)
