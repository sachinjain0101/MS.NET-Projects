Create PROCEDURE [dbo].[usp_APP_GenericPayrollUpload_GetRecs_Standard_BONDADAPT_GetData]
(
    @Client         VARCHAR(4),
    @GroupCode      INT,
    @PPED           DATETIME,
...
UPDATE #tmpExport SET [SourceTime]= CASE WHEN IVR_Count>0 THEN 'I'
                    ...
                    , [SourceApprove]= CASE WHEN FAXApprover_Count>0 THEN 'F'
                                       ...
                                       END
UPDATE #tmpExport
  SET Line1 = ISNULL(AssignmentNo, ' ') + @Delim
            + ISNULL(EmplFirst, ' ') + ' ' + ISNULL(EmplLast, ' ') + @Delim 
            + ISNULL(EmployeeID, ' ')   + @Delim
            + @Delim
            + @Delim
						+ '1' + @Delim   -- ISNULL(SourceTime, ' ')   + @Delim
