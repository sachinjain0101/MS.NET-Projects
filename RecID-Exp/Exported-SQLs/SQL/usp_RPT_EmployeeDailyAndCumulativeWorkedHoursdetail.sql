...
--GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.usp_RPT_EmployeeDailyAndCumulativeWorkedHoursDetail') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE dbo.usp_RPT_EmployeeDailyAndCumulativeWorkedHoursDetail AS' 
END 
 GO

ALTER PROCEDURE dbo.usp_RPT_EmployeeDailyAndCumulativeWorkedHoursDetail
(
	 @Client CHAR(4)
	,@Group INT
	,@Date DATETIME
