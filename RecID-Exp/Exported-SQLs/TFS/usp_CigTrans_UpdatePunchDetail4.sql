Create PROCEDURE [dbo].[usp_CigTrans_UpdatePunchDetail4](
     @Client char(4)
   , @GroupCode int
...
DECLARE @ConnectedOutTime datetime
...
BEGIN
	...
	BEGIN
		SET @ConnectedOutTime = (SELECT dbo.PunchDateTime2(transDate, outDay, OutTime)
		...
		IF @ConnectedOutTime IS NOT NULL
			SET @InTransDateTime = @ConnectedOutTime --reset @intransdatetime to be same as outtime of connected transaction
	END
END
...
Set GroupCode = @GroupCode
  ...
  , OutTimeStamp = @OutTimeStamp
	, HandledByImporter = (CASE WHEN @FixPunchStatus IN ('1','2') THEN 'V' ELSE HandledByImporter END) 
  , InRoundOFF = @InRoundOFF
  , OutRoundOFF = @OutRoundOFF
Where RecordID = @THDRecordId

DECLARE @StdJobID AS BIGINT  --< @StdJobId data type is changed from  INT to BIGINT by Srinsoft on 04Oct2016 >--

IF (SELECT ISNULL(AllowLaborTrack, '0') FROM TimeCurrent..tblClientGroups WITH(NOLOCK)
		WHERE client = @Client
		AND GroupCode = @GroupCode) = '1'
BEGIN
	
	SET @StdJobID = (
		SELECT distinct jobID
		FROM tblstdjobcellemployees WITH(NOLOCK)
		WHERE Client= @Client
		AND   GroupCode= @GroupCode
		AND   SSN= @SSN
		AND   ISNULL(EndDateTime, '1/1/2999') =  '1/1/2999'
		AND   EffectiveDateTime = @ActualInTime
	)
	IF @StdJobID IS NOT NULL
	BEGIN
		UPDATE tblstdjobcellemployees 
		SET EndDateTime= @ActualOutTime
		WHERE Client= @Client
		AND   GroupCode= @GroupCode
		AND   SSN= @SSN
		AND   ISNULL(EndDateTime, '1/1/2999') =  '1/1/2999'
