CREATE PROCEDURE [dbo].[usp_APP_GenericPayrollUpload_GetRecs_HSSS_PCT]
(
  @Client       varchar(4),
  @GroupCode    int,
  @PPED         datetime,
  ...
  SELECT 	
    ...
    FROM TimeHistory.dbo.tblTimeHistDetail_UDF as udf
        INNER JOIN TimeCurrent.dbo.tblEmplNames as en
            ...
            AND	[en].SSN = [udf].SSN
        Left JOIN TimeCurrent..tblEmplAssignments as ea
          ON  ea.Client = udf.Client
          AND ea.GroupCode = udf.GroupCode
          AND ea.SSN = udf.SSN
          AND ea.DeptNo = udf.DeptNo
...
UPDATE #tmpExport  SET AssignmentNo = 'MISSING' WHERE isnull(AssignmentNo,'') = ''
...
Insert into #tmpExport(SSN,EmpName,AssignmentNo,FileBreakID, Line1 )
select DISTINCT 1,'1','MISSING', FileBreakID,
...
SELECT * FROM #tmpExport 
    ORDER BY FileBreakID,
             CASE WHEN AssignmentNo = 'MISSING'
                  THEN '0' 
                  ELSE '1' 
             END
           , EMPNAME
