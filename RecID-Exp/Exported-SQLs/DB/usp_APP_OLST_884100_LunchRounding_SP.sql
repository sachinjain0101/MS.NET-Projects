CREATE     Procedure [dbo].[usp_APP_OLST_884100_LunchRounding_SP]
(
  @Client varchar(4),
  @GroupCode int,
  @PPED datetime,
...
BEGIN
	IF (@@fetch_status <> -2)
	BEGIN
    -- If the Lunch Break is 37 mins or more than add 37 mins back.  Under 37 mins, add back actual
    Set @TotHours = case when @Minutes > 37 then 0.62 else round(@Minutes/60.0,2) end
    Set @RecordID = NULL
