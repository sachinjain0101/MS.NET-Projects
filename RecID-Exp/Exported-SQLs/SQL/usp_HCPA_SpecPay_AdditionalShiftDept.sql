USE [TimeHistory]
GO
/****** Object:  StoredProcedure [dbo].[usp_HCPA_SpecPay_AdditionalShiftDept]    Script Date: 11/3/2015 9:13:50 AM ******/
SET ANSI_NULLS ON
GO
...
select deptNo, MasterDept,DeptName,ClientDeptCode,CLientDeptCode2 from TimeCurrent..tblGroupDepts where client = 'HCPA' and groupcode = 550010
...
DECLARE @thdRecordID BIGINT  --< @thdRecordId data type is changed from  INT to BIGINT by Srinsoft on 25Aug2016 >--
...
Insert into @tmpRec
select t.RecordID, t.InClass, gd.Clientdeptcode, gd.MasterDept, t.Dollars , t.CostID, t.DeptNo, d.Payrate   
...
BEGIN
	...
	READ_ONLY
	FOR 
	select RecordID, InClass, Clientdeptcode, MasterDept, Dollars, CostID, DeptNo, Payrate   
	from @tmpRec 

	OPEN cTHDSum

	FETCH NEXT FROM cTHDSum INTO @thdRecordID, @InClass, @RateCode, @newCostID, @curDollars, @curCostID, @DeptNo, @PayRate
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN	
				...
				where RecordID = @thdRecordID 
	 				and ( isnull(CostID,'') <> Left(@newCostID,30) or isnull(ShiftDiffAmt,0) <> @PayRate )
