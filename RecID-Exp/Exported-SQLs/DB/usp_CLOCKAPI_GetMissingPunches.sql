CREATE Procedure [dbo].[usp_CLOCKAPI_GetMissingPunches]
(
  @TermID varchar(20),
  @Version varchar(20),
  @Request varchar(20),
...
BEGIN
    ...
    AND (
        ...
        AND (
          ...
          (SELECT MAX(dbo.PunchDateTime2(t2.TransDate, t2.InDay, t2.InTime)) 
           FROM TimeHistory..tblTimeHistDetail as t2 with (nolock)
           WHERE t2.Client = thd.Client 
           AND t2.GroupCode = thd.GroupCode 
           AND t2.SSN = thd.SSN 
           AND t2.PayrollPeriodEndDate = thd.PayrollPeriodEndDate)
	)
      )
    )
    ...
    BEGIN
      ...
      BEGIN
        ...
        IF @TermID = '4311340001'
        BEGIN
          Set @SPROC = 'RecordID = ' + ltrim(str(@RecordID)) + ', InDateTime = ' + case when @InPunch is null then '' else CONVERT(varchar(32),@InPunch, 120) end 
