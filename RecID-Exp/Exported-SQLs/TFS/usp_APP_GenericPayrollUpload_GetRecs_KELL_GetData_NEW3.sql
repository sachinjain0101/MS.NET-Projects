Create PROCEDURE [dbo].[usp_APP_GenericPayrollUpload_GetRecs_KELL_GetData_NEW3]
(
  @Client         varchar(4),
  @GroupCode      int,
  @PPED           datetime,
...
BEGIN
    ...
    SELECT 
       ...
       , OTOverride = 0 /*SUM(CASE WHEN TimeHistory.dbo.fn_DoesOTOExist (@Client, @GroupCode, t.SSN,	t.PayrollPeriodEndDate) = 1 OR 
       ...
       , Last5SSN = ( SELECT TOP 1 SSN
                      FROM TimeCurrent.dbo.tblRFR_Empls rfr WITH(NOLOCK)
    ...
    INNER JOIN TimeCurrent..tblClientGroups as tc_cg WITH(NOLOCK)
    ...
    /*LEFT JOIN TimeHistory..tblWTE_TimeSheets ts
        ON ts.EmployeeID = tc_en.RecordID
        AND ts.PayPeriodEndDate = t.PayrollPeriodEndDate
    ...
    WHERE   t.Client = @Client
        ...
        AND ISNULL(ac.ADP_HoursCode, '') NOT IN ('26', '572', '581', '584')
    GROUP BY
          t.SSN
        , t.SiteNo
        , t.DeptNo
...
UPDATE #tmpAssignments 
SET OTOverride = 1
WHERE TimeHistory.dbo.fn_DoesOTOExist (@Client, @GroupCode, SSN,	@PPED) = 1 
OR ISNULL(AgencyName, '') <> ''
OR (BranchID IN ('35C3','3547') AND ClientID = '01327464')
OR SiteState = 'PR'
...
INNER JOIN TimeCurrent.dbo.tblEmplNames_Depts as ed  WITH(NOLOCK)
ON  ed.Client = hd.Client
AND	ed.GroupCode = hd.GroupCode
AND	ed.SSN = hd.SSN
AND ed.Department = hd.DeptNo
INNER JOIN TimeHistory.dbo.tblEmplNames_Depts as edh  WITH(NOLOCK)
...
INNER JOIN TimeHistory.dbo.tblEmplSites_Depts as esdh  WITH(NOLOCK)
...
INNER JOIN TimeCurrent.dbo.tblEmplSites_Depts as esd  WITH(NOLOCK)
ON  esd.Client = hd.Client
AND esd.GroupCode = hd.GroupCode
AND esd.SSN = hd.SSN
AND esd.DeptNo = hd.DeptNo    
AND esd.SiteNo = hd.SiteNo    
INNER JOIN TimeCurrent.dbo.tblAdjCodes as ac  WITH(NOLOCK)
...
LEFT JOIN TimeCurrent.dbo.tblAgencies as ag  WITH(NOLOCK)
ON  ag.Client = en.Client
AND ag.GroupCode = en.GroupCode 
AND ag.Agency = en.AgencyNo 
LEFT JOIN TimeCurrent..tblPayRules pr  WITH(NOLOCK)
ON pr.RecordID = enh.PayRuleID
LEFT JOIN TimeCurrent..tblEntryRoundingRules err  WITH(NOLOCK)
