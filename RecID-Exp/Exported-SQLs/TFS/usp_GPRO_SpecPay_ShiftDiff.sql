Create PROCEDURE [dbo].[usp_GPRO_SpecPay_ShiftDiff]
(
  @Client varchar(4),
  @GroupCode int,
  @PPED datetime,
...
DECLARE @savRecordID BIGINT  --< @savRecordId data type is changed from  INT to BIGINT by Srinsoft on 24Aug2016 >--
...
--BEGIN
  ...
  InClass, OutClass 
  from Timehistory..tblTimeHistDetail (NOLOCK)
  where client = @Client
...
and Payrollperiodenddate = @PPED
and clockadjustmentno = ''
and DeptNo in(select DeptNo from TimeCurrent..tblDeptShiftDiffs (NOLOCK) where Client = @Client and GroupCode = @GroupCode and SiteNo = tblTimeHistDetail.SiteNo )
...
BEGIN
    ...
    BEGIN
      ...
      from TimeCurrent.dbo.tblDeptShiftDiffs  (NOLOCK)
      where client = @Client
      and GroupCode = @Groupcode
      and SiteNo = @SiteNo
      and DeptNo = @DeptNo
    ...
    BEGIN
      ...
      Update TimeHistory..tblTimeHistDetail 
        Set ShiftNo = @PrevShiftNo, 
            ShiftDiffClass = @PrevShiftNo, 
            ShiftDiffAmt = @PrevShiftDiffAmt 
        where RecordID = @RecordID
    ...
    Select @NewShiftNo = ShiftNo, 
      ...
      Set @TransDate = (select Transdate From TimeHistory..tblTimeHistDetail (NOLOCK) WHERE recordid = @RecordID )
      Update TimeHistory..tblTimeHistDetail Set ShiftNo = @NewShiftNo,ShiftDiffAmt = @ShiftDiffAmt,ShiftDiffClass = @NewShiftNo where RecordID = @RecordID
      Update TimeHistory..tblTimeHistDetail
        Set ShiftNo = @NewShiftNo,
            ShiftDiffAmt = @ShiftDiffAmt,
            ShiftDiffClass = @NewShiftNo
      where client = @Client and groupcode = @groupcode and ssn = @SSN and TransDate = @TransDate and ClockADjustmentno = '8'
