Create PROCEDURE [dbo].[usp_APP_DAVT_CA_BreakPenalty_SpecPay]
(
  @Client varchar(4),
  @GroupCode int,
  @PPED datetime,
...
DECLARE @PilotFlag char(1) 
DECLARE @PromptBreak char(1)

-- New Rules to handle Break exceptions are being put in place for 2013.
-- Any pay period after 1/5/2013 needs to include the new rules.
--
if @PPED > '1/5/13'
BEGIN
  EXECUTE [TimeHistory].[dbo].[usp_APP_DAVT_CA_BreakPenalty_SpecPay_New] @Client,@GroupCode,@PPED,@SSN
  RETURN
END
...
and ClockAdjustmentNo = @AdjCode and InSrc = '3' and UserCode = 'SYS' and (AdjustmentName like 'INTRNL%' or AdjustmentName like 'NMR%')
and TransType <> 7 
...
READ_ONLY
FOR 
select t.RecordID, PromptBreak = isnull(sn.PromptBreak,'0'),
...
FETCH NEXT FROM cTHD1 INTO @RecordID, @PromptBreak, @ClockInTime, @ClockOutTime, @InClass, @OutClass, @AdjNo
...
-- Pilot for 509100
--
if (@GroupCode = 509100 and @PPED >= '12/1/12' and @PilotFlag = '1' ) OR (@GroupCode = 509200 and @PPED >= '12/15/12' and @PilotFlag = '1')
BEGIN
  EXECUTE [TimeHistory].[dbo].[usp_APP_DAVT_CA_BreakPenalty_SpecPay_New] @Client,@GroupCode,@PPED,@SSN
  Return 
END
