CREATE Procedure [dbo].[usp_APP_DAVT_CA_BreakPenalty_SpecPay_New]
(
  @Client varchar(4),
  @GroupCode int,
  @PPED datetime,
...
DECLARE @adjcode varchar(3)  --< Srinsoft 08/07/2015 Changed @adjcode char(1) to varchar(3) >--
...
DECLARE @SiteNo int
DECLARE @DeptNo int
DECLARE @RecordID BIGINT  --< @RecordID data type is converted from INT to BIGINT by Srinsoft on 01Aug2016 >--
...
DECLARE @savRecordID BIGINT  --< @savRecordID data type is converted from INT to BIGINT by Srinsoft on 01Aug2016 >--
...
DECLARE @AdjNo varchar(3) --< Srinsoft 08/07/2015 Changed @AdjNo char(1) to varchar(3) >--
...
(
	RecordID BIGINT,  --< RecordID data type is converted from INT to BIGINT by Srinsoft on 01Aug2016 >--
...
BEGIN
    ...
    Set @savBreakMins = 0
		...
		BEGIN
			Insert into @tmpPenalty (SSN, TransDate, FirstPunch, LastPunch, Type) values (@SSN, @TransDate, @FirstPunch, @LastPunch, case when @shortLunchFlag = '1' then 'SH_MLBRK-W' else 'NO_M2BRK-W' end)
		END		
		-- If this location did not waive 2nd break and two lunch breaks were not taken and the hours > 10.00 
		-- and the manual break amount < .50 then Penalty should apply
		IF @LunchBreakCount < 3 and (@TotWorked - @TotBreakHrs) >= 15.00 and @TotBreakHrs < 1.50
		BEGIN
			Insert into @tmpPenalty (SSN, TransDate, FirstPunch, LastPunch, Type) values (@SSN, @TransDate, @FirstPunch, @LastPunch, case when @shortLunchFlag = '1' then 'SH_M3BRK-W' else 'NO_M3BRK-W' end)
		END		
      ...
      Begin  
        Insert into @tmpPenalty (SSN, TransDate, FirstPunch, LastPunch, Type) values (@SSN, @TransDate, @FirstPunch, @LastPunch, case when @shortLunchFlag = '1' then 'SH_MLBRK-W' else 'NO_MLBRK-W' end)
      End
		END		
	END
	NextRecord:
...
BEGIN
    ...
    IF @Type = 'NO_MLBRK-W'
      Set @oldAdjName = 'NMR_NB'
    IF @Type = 'NO_M2BRK-W'
      Set @oldAdjName = 'NMR_2B'
    IF @Type = 'LT_MLBRK-W'
      Set @oldAdjName = 'NMR_SL'
    ...
    select 
      ...
      @LastPunch = LastPunch
    from @tmpPenalty where Transdate = @TransDate and Type = @Type
...
-- Delete Break Exceptions for dates that no longer have a penalty
--
/*
-- Clear work table 
delete from @tmpTransDateMP

-- Get dates that have exception
--
Insert into @tmpTransDateMP (TransDate)
Select Distinct TransDate from TImeHistory..tblTimeHistDetail with (nolock)
where client = @Client
and groupcode = @Groupcode 
and PayrollPeriodEndDate = @PPED 
and SSN = @SSN 
and ClockAdjustmentNo in('N','/')


-- Remove exceptions from the break exception table for the dates that no longer have any exception
--
...
and PayrollPeriodEndDate = @PPED 
and SSN = @SSN 
and transdate not in(select Transdate from @tmpTransDateMP)
*/
