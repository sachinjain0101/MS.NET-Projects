...
--GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.usp_RPTPayCodeSelection_YTD') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE dbo.usp_RPTPayCodeSelection_YTD AS' 
END 
 GO


ALTER    PROCEDURE [dbo].[usp_RPTPayCodeSelection_YTD]
(
