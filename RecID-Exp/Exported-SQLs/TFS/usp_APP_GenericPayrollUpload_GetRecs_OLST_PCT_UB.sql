Create PROCEDURE [dbo].[usp_APP_GenericPayrollUpload_GetRecs_OLST_PCT_UB]
(   
  @Client         char(4),  
  @GroupCode      int,  
  @PPED           DateTime,  
...
DECLARE @SSN                INT  
DECLARE @ProjectGroupCode   INT   
DECLARE @ProjectPPED        DATETIME   
...
DECLARE @AdditionalFields   VARCHAR(1)  
...
(  
    ...
    ApproverName         VARCHAR(100),  
    ApproverDateTime     DATETIME,  
    ApprovalID           BIGINT,  --< ApprovalId data type is changed from  INT to BIGINT by Srinsoft on 09Aug2016 >--
...
WHERE (TotalRegHours <> 0) OR (NoHours = '1')  
...
SELECT DISTINCT
      ...
      , Line1 =  
        ...
        '' + @Delim  + -- 'DAYWORKED'  
          ...
          CONVERT(VARCHAR, EmplName) + @Delim +  -- 'EMPLOYEE_NAME'
					'' + @Delim +  -- 'XREF_SYSTEMID'  
					ISNULL(CAST(CAST(CAST(tmpWorkedSummary.BillRate AS DECIMAL(6,2)) * CAST(ISNULL(tcESD.BillingOvertimeCalcFactor,1.50) AS DECIMAL(6,2))AS DECIMAL(6,2)) AS VARCHAR(6)),'') + @Delim +  -- 'VMS_BILL_RATE'
					'' + @Delim +  -- 'VMS_VENDOR_PAY_RATE'  
					CONVERT(VARCHAR, FileNo) + -- 'EMPLID'
          CASE WHEN @AdditionalFields = '0' THEN ''  
                                            ELSE
