...
DECLARE @Bereavement NUMERIC(7,2)
DECLARE @JuryDuty NUMERIC(7,2),
				@AdminPTO NUMERIC(7,2),
				@AdminUNPTO NUMERIC(7,2),
				@NonWorkPTO NUMERIC(7,2)
...
--AND TransDate < '1/1/2017'			-- exclude any holidays prior to 1/1/2017.
...
and t.groupcode = @GroupCode 
and t.payrollperiodenddate in(@PPED_1, @PPED_2)
and t.clockadjustmentno in('2','5','P','3','4','2AL','2AU','2NW' ) 
...
BEGIN
    ...
    if exists(select recordid from TimeHistory..tblTimeHistDetail with (nolock) 
							WHERE client = @Client and groupcode = @Groupcode
									AND SSN = @SSN and clockadjustmentno in('2','5','P','3','4','2AL','2AU','2NW' ) and Payrollperiodenddate = @PPED
...
DOITAGAIN:
			...
			--
			Select @WeekTot = Sum(case when clockadjustmentno not in('>','<','-','2UP','2CG','2LT','2D0','2SC','SUP') then hours else 0.00 end ),
             @RegHours = sum(case when RegHours <> 0.00 and clockadjustmentno not in('>','<','-','2UP','2CG','2LT','2D0','2SC','SUP') then RegHours else 0.00 end),
             ...
             @DTHours = sum(DT_Hours),
						 ...
						 @PTOTaken = Sum(Case when ClockAdjustmentNo = '2' then Hours else 0.00 end ),
						 ...
						 @uPTOTaken = Sum(Case when ClockAdjustmentNo = 'P' then Hours else 0.00 end ),
             @UnPaid = sum(case when clockadjustmentno in('>','<','-') and UserCode = 'SYW' then Hours else 0.00 end),
						 @Bereavement = Sum(Case when ClockAdjustmentNo = '3' then Hours else 0.00 end ),
						 @Juryduty =  Sum(Case when ClockAdjustmentNo = '4' then Hours else 0.00 end ),
						 @AdminPTO =  Sum(Case when ClockAdjustmentNo = '2AL' then Hours else 0.00 end ),
						 @AdminUNPTO =  Sum(Case when ClockAdjustmentNo = '2AU' then Hours else 0.00 end ),
						 @NonWorkPTO =  Sum(Case when ClockAdjustmentNo = '2NW' then Hours else 0.00 end )
      ...
      BEGIN
        ...
        Delete from TimeHistory..tblTimeHistDetail where client = @Client and groupcode = @groupcode
          and PayrollPeriodenddate = @PPED and ssn = @SSN
          and ClockAdjustmentNo in('2','5','P','3','4') and UserCode = 'SYW' and Hours <= 0.00
        IF @SubStatus4 <> 'L'
...
--			IF @FullTime <> 'P' 
      ...
      END
			...
			IF isnull(@WeekTot,0) <= @BaseHours 
					OR (@PTOTaken + @uPTOTaken + @EILTaken + @Bereavement + @JuryDuty + @AdminPTO + @AdminUNPTO + @NonWorkPTO) <= 0.00
			BEGIN
				GOTO NEXT_RECORD
			END
      ...
      Values(@GroupCode, @SSN, '', @BaseHours, @WeekTot,(@PTOTaken + @uPTOTaken),@EILTaken,NULL, 'Recalc:Week Tot > Base Hours', @PPED)
			...
			-- AdminPTO
		  --
			IF @AdminPTO > 0 AND @PTOBalance1 > 0
			BEGIN
				EXEC TimeHistory.[dbo].[usp_WEB1_DAVT_CheckUnpaidPTO_Weekly_Helper] @Client, @GroupCode, @SSN, @PPED, @MPD, @PTOBalance1, @AdminPTO, '2AL', 'AdmPTO*', @AuditRecordID

				IF @AdminPTO> @PTOBalance1
				BEGIN
    			Set @PTOBalance1 = 0
				END
				ELSE
				BEGIN
    			Set @PTOBalance1 = @PTOBalance1 - @AdminPTO
				END
			END
			-- AdminUNPTO
		  --
			IF @AdminUNPTO > 0 AND @PTOBalance1 > 0
			BEGIN
				EXEC TimeHistory.[dbo].[usp_WEB1_DAVT_CheckUnpaidPTO_Weekly_Helper] @Client, @GroupCode, @SSN, @PPED, @MPD, @PTOBalance1, @AdminUNPTO, '2AU', 'AdmUNPTO*', @AuditRecordID

				IF @AdminUNPTO > @PTOBalance1
				BEGIN
    			Set @PTOBalance1 = 0
				END
				ELSE
				BEGIN
    			Set @PTOBalance1 = @PTOBalance1 - @AdminUNPTO
				END
			END
			-- NonWorkPTO
		  --
			IF @NonWorkPTO > 0 AND @PTOBalance1 > 0
			BEGIN
				EXEC TimeHistory.[dbo].[usp_WEB1_DAVT_CheckUnpaidPTO_Weekly_Helper] @Client, @GroupCode, @SSN, @PPED, @MPD, @PTOBalance1, @NonWorkPTO, '2NW', 'NW-PTO*', @AuditRecordID

				IF @NonWorkPTO > @PTOBalance1
				BEGIN
    			Set @PTOBalance1 = 0
				END
				ELSE
				BEGIN
    			Set @PTOBalance1 = @PTOBalance1 - @NonWorkPTO
				END
			END
		  ...
		  --
			...
			IF @Bereavement > 0 AND @PTOBalance1 > 0
			BEGIN
				EXEC TimeHistory.[dbo].[usp_WEB1_DAVT_CheckUnpaidPTO_Weekly_Helper] @Client, @GroupCode, @SSN, @PPED, @MPD, @PTOBalance1, @Bereavement, '3', 'BEREAVE*', @AuditRecordID

				IF @Bereavement > @PTOBalance1
				BEGIN
    			Set @PTOBalance1 = 0
				END
				ELSE
				BEGIN
    			Set @PTOBalance1 = @PTOBalance1 - @Bereavement
				END
			END


			IF @JuryDuty > 0 AND @PTOBalance1 > 0
			BEGIN
				EXEC TimeHistory.[dbo].[usp_WEB1_DAVT_CheckUnpaidPTO_Weekly_Helper] @Client, @GroupCode, @SSN, @PPED, @MPD, @PTOBalance1, @JuryDuty, '4', 'JURYDUTY*', @AuditRecordID
				IF @JuryDuty > @PTOBalance1
				BEGIN
    			Set @PTOBalance1 = 0
				END
				ELSE
				BEGIN
    			Set @PTOBalance1 = @PTOBalance1 - @JuryDuty
				END
			END
