CREATE Procedure [dbo].[usp_WEB1_DAVT_CheckUnpaidPTO] 
(
	@Client varchar(4),
	@GroupCode int,
	@PPED_2 datetime,
...
BEGIN
  ...
  (
    ...
    Dateadded datetime,
    Hours numeric(9,2),
    ClockAdjustmentNo varchar(3), --< Srinsoft 09/09/2015 Changed ClockAdjustmentNo char(1) to varchar(3) for #tmpHol >--
    UserCode varchar(5)
  )
...
DECLARE @CashoutDate datetime
Set @CashoutDate = dateadd(day,6,@PPED_2) -- Cash Out date is the Friday after the Master period end date

-- Create table for the Cash outs for this pay period.
--
Select SSN, CashOutAmount = sum(Hours)
into #tmpCashOuts
from TimeHistory..tblPTORequests as p with(nolock)
where client = @Client
and GroupCode = @Groupcode
and TransDate = @CashoutDate 
and [Status] <> 'X'
group by SSN 
...
CashOutAmount = isnull(c.CashOutAmount,0) 
from TimeHistory..tblTimeHistDetail as t with (nolock)
...
Group By t.Client, t.GroupCode, t.SSN, t.MasterPayrolldate, 
a.Balance, 
a.effWeekEndDate,
c.CashOutAmount 
Having sum(case when t.ClockADjustmentNo IN('P','2') then t.Hours else 0.00 end) + isnull(c.CashOutAmount,0) > a.Balance
...
Declare @CashOutAmt numeric(7,2)


OPEN cSSNS

FETCH NEXT FROM cSSNS INTO @Client, @Groupcode, @SSN, @MPD, @WeekLocked, @PTOTaken, @uPTOTaken, @PTOTaken2, @PTOBalance, @EffDate, @CashOutAmt
WHILE (@@fetch_status <> -1)
BEGIN
	IF (@@fetch_status <> -2)
	BEGIN
      ...
      Values(@GroupCode, @SSN, 'Balances', @PTOBalance, @EILBalance, 0,0,@EffDate, '')
			...
			Set @PTOBalance = @PTOBalance - @CashOutAmt 
    	...
    	--
    	IF @MPD <> @PPEDStart and @MPD <> @PPEDEnd
      BEGIN
        insert into TimeCurrent..tblWork_DAVT_UnpaidPTO_Audit(GroupCode, SSN, Step, PTO, EIL, PTOTaken, EILTaken, EffDate, Message)
      ...
      insert into TimeCurrent..tblWork_DAVT_UnpaidPTO_Audit(GroupCode, SSN, Step, PTO, EIL, PTOTaken, EILTaken, EffDate, Message)
      Values(@GroupCode, @SSN, 'Balances 2', @PTOBalance, @EILBalance, (@PTOTaken + @uPTOTaken + @PTOTaken2 + @uPTOTaken2 + @CashoutAmt),@CashoutAmt,@EffDate, '')
      ...
      BEGIN
      	...
      	BEGIN
      		...
      		EXEC [TimeHistory].[dbo].[usp_APP_XLSImport_Adjustment_Insert_THD] @Client, @GroupCode, @PPEDStart, @SSN, 0, 0, 'P', 'UNSCPTO', @TempHours, 0.00, @PPEDStart, @PPEDEnd, 'SYS', 'N','N','N','1'
      		EXEC [TimeHistory].[dbo].[usp_APP_XLSImport_Adjustment_Insert_THD] @Client, @GroupCode, @PPEDStart, @SSN, 0, 0, '<', 'UnPD UnSch', @uPTOTaken, 0.00, @PPEDStart, @PPEDEnd, 'SYS', 'N','N','N','1'
        ...
        if exists (Select 1 from Timehistory..tblTimeHistDetail with(nolock)
            ...
            and groupcode = @Groupcode
            and SSN = @SSN
            and Payrollperiodenddate = @PPEDStart
      ...
      BEGIN
      	...
      	BEGIN
      		...
      		EXEC [TimeHistory].[dbo].[usp_APP_XLSImport_Adjustment_Insert_THD] @Client, @GroupCode, @PPEDStart, @SSN, 0, 0, '2', 'PTO', @TempHours, 0.00, @PPEDStart, @PPEDEnd, 'SYS', 'N','N','N','1'
      		EXEC [TimeHistory].[dbo].[usp_APP_XLSImport_Adjustment_Insert_THD] @Client, @GroupCode, @PPEDStart, @SSN, 0, 0, '>', 'UnPD PTO', @PTOTaken, 0.00, @PPEDStart, @PPEDEnd, 'SYS', 'N','N','N','1'
          insert into TimeCurrent..tblWork_DAVT_UnpaidPTO_Audit(GroupCode, SSN, Step, PTO, EIL, PTOTaken, EILTaken, EffDate, Message, PPED)
      ...
      END
			...
			-- Update Totals for second week check.
			-- 
    	Select @PTOTaken = sum(case when t.ClockADjustmentNo = '2' and t.PayrollPeriodEndDate = @PPEDStart then t.Hours else 0.00 end),
    				 @uPTOTaken = sum(case when t.ClockADjustmentNo = 'P' and t.PayrollPeriodEndDate = @PPEDStart then t.Hours else 0.00 end),
    	       @PTOTaken2 = sum(case when t.ClockADjustmentNo = '2' and t.PayrollPeriodEndDate = @PPEDEnd then t.Hours else 0.00 end),
      ...
      BEGIN
      	...
      	BEGIN
      		...
      		EXEC [TimeHistory].[dbo].[usp_APP_XLSImport_Adjustment_Insert_THD] @Client, @GroupCode, @PPEDEnd, @SSN, 0, 0, 'P', 'UNSCPTO', @TempHours, 0.00, @PPEDEnd, @PPEDEnd, 'SYS', 'N','N','N','1'
      		EXEC [TimeHistory].[dbo].[usp_APP_XLSImport_Adjustment_Insert_THD] @Client, @GroupCode, @PPEDEnd, @SSN, 0, 0, '<', 'UnPD UnSch', @uPTOTaken2, 0.00, @PPEDEnd, @PPEDEnd, 'SYS', 'N','N','N','1'
          insert into TimeCurrent..tblWork_DAVT_UnpaidPTO_Audit(GroupCode, SSN, Step, PTO, EIL, PTOTaken, EILTaken, EffDate, Message,PPED)
        ...
        if exists (Select 1 from Timehistory..tblTimeHistDetail with(nolock)
            ...
            and groupcode = @Groupcode
            and SSN = @SSN
            and Payrollperiodenddate = @PPEDEnd
            and ClockAdjustmentNo = 'P'
            and isnull(AprvlStatus,'') = 'A')
        BEGIN
          Update Timehistory..tblTimeHistDetail
      ...
      BEGIN
      	...
      	BEGIN
      		...
      		EXEC [TimeHistory].[dbo].[usp_APP_XLSImport_Adjustment_Insert_THD] @Client, @GroupCode, @PPEDEnd, @SSN, 0, 0, '2', 'PTO', @TempHours, 0.00, @PPEDEnd, @PPEDEnd, 'SYS', 'N','N','N','1'
      		EXEC [TimeHistory].[dbo].[usp_APP_XLSImport_Adjustment_Insert_THD] @Client, @GroupCode, @PPEDEnd, @SSN, 0, 0, '>', 'UnPD PTO', @PTOTaken2, 0.00, @PPEDEnd, @PPEDEnd, 'SYS', 'N','N','N','1'
          insert into TimeCurrent..tblWork_DAVT_UnpaidPTO_Audit(GroupCode, SSN, Step, PTO, EIL, PTOTaken, EILTaken, EffDate, Message, PPED)
