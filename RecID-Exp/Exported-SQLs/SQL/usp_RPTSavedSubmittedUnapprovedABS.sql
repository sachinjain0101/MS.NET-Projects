...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.usp_RPTSavedSubmittedUnapprovedABS') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE dbo.usp_RPTSavedSubmittedUnapprovedABS AS' 
END 
 GO

ALTER PROCEDURE  dbo.usp_RPTSavedSubmittedUnapprovedABS
  (   @DateFrom DATETIME
	  , @DateTo DATETIME
    , @Date DATETIME
    , @Client VARCHAR(4)
