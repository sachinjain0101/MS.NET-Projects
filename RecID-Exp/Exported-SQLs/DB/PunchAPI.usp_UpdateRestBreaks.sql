CREATE PROCEDURE [PunchAPI].[usp_UpdateRestBreaks]
(
  @Client VARCHAR(4) ,
  @Groupcode INT ,
  @EmplBadge INT ,
...
SET NOCOUNT ON
    ...
    BEGIN
      ...
      IF @PenaltyBreaks > 0
      BEGIN
        SET @Comment = 'REST BREAK RESPONSE - Warning: Time card NOT updated.  System could not find matching out punch for Out Punch (' + CONVERT(VARCHAR(16), @punchDateTime, 120) + '). Employee recorded number of missed rest breaks = ' + LTRIM(STR(@PenaltyBreaks)) + ''
      ...
      END
			SELECT Client = @Client, GroupCode = @Groupcode, PPED = @PPED, SSN = @SSN WHERE 1 = 0
      return  
    END
