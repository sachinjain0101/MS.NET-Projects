Create PROCEDURE [dbo].[usp_RPT_GroupAudit2] 
( 
  @Client char(4), 
  @Group int, 
  @Date DateTime, 
  @Sites varchar(1000) = NULL 
) 
AS 
...
DECLARE @RecID BIGINT  --< @RecId data type is changed from  INT to BIGINT by Srinsoft on 30Aug2016 >--
DECLARE @SiteNo int
...
DECLARE @EmployeeCountLast2Wks int
...
Set @TempPPED = dateadd(day, -14, @PPED)
Set @EmployeeCountLast2Wks = (SElect Count(Distinct SSN) from TimeHistory..tblTimeHistDetail where client = @Client and Groupcode = @Group and PayrollPeriodenddate >= @tempPPED and Payrollperiodenddate < @PPED)
...
DECLARE @EmployeesInLast2WeekCount VARCHAR(MAX);
 SELECT
 @EmployeesInLast2WeekCount = STUFF((SELECT DISTINCT ' - ' + '"' + tcEN.FirstName + ' ' + tcEN.LastName + '"'
...
values(2.3,'Week Ending','Active Empl Count','Empl Not Punching','New Empls','Empl Count Last 2 Wks',
...
ltrim(str(@NewEmpls)),
ltrim(str(@EmployeeCountLast2Wks)),
...
-- Employee Names ... from 2 Week Employee Count 
INSERT INTO #tmpReport (SortID,Col1)
VALUES(2.41,'');
INSERT INTO #tmpReport (SortID,Col1)
SELECT 2.42,'Employees: ' + @EmployeesInLast2WeekCount ;
